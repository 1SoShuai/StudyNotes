...
​		还是前面的*`1.txt`*，修改内容后已经提交到暂存区了，这时我们想撤销刚才的操作，可以使用`git reset head 1.txt`指令（该命令默认不会更改工作目录）。

#### 4.9.3撤销提交更改

​		有时候我们意识到刚提交的内容不正确想要撤销，有几种方式可以处理这种问题，下面这种是安全的方式，不会造成数据丢失，实际上我们将通过**创建新的提交来撤销原来需要更改的提交。**例如执行`git revert head`

![](imgs/revert.png)

假设head刚开始指向的是版本三，执行完revert后，将生成一个版本四，这个版本四的状态与版本二是相同的。如果我们想要回退到版本三，可以先通过`git reflog`找到commit id，然后`git reset --hard commitID`,这样就可以回到我们想要的版本了。
